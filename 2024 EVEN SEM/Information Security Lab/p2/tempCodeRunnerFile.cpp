...
void parse(const vector<Token>& tokens) {
    ...
    while (index < tokens.size()) {
        ...
        cout << "State: " << state << " | Token: " << token.value << " | Line: " << token.line << " | Action: " << action << endl;
        ...
        if (action[0] == 's') {
            ...
            cerr << "Syntax error at line " << token.line << ": " << token.value << endl;
            return;
        }
    }
}
...
int main() {
    ...
    if (!inputFile.is_open()) {
        cerr << "Error: Unable to open file sample.txt" << endl;
        return 1;
    }
