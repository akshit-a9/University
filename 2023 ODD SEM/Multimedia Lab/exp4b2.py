def lz77_compress(input_string, window_size, lookahead_buffer_size):
    ...
    while i < len(input_string):
        ...
        for j in range(1, min(lookahead_buffer_size, len(input_string) - i) + 1):
            ...
            prev_occurrence = input_string[max(0, i - window_size):i]
            if substring in prev_occurrence:
                match = substring
                offset = i - prev_occurrence.rfind(substring)
                length = len(match)
...
def lz77_decompress(compressed_data):
    ...
    for (offset, length, char) in compressed_data:
        if offset == 0:
            ...
            start = max(0, len(decompressed_string) - offset)
            for i in range(length):
                if start + i < len(decompressed_string):
